...
LibreOffice on the app server. Put `packages.txt` at the actual GitHub repository
root. A copy only inside a nested `final` or `Streamlit` folder is insufficient.
