...
class DesktopUITests(unittest.TestCase):
    def test_recovered_request_visible_without_switching_chats(self):
        import chat_requests
        self.enter()
        self.app.button(key='nav_chat').click().run()
        session_id = self.app.session_state.active_chat_session_id
        path = self.root / 'dms_system.db'
        self.assertTrue(chat_requests.begin(path, 'interrupted', session_id, 1, 'Saved interrupted question'))
        with closing(database.open_database()) as conn, conn:
            conn.execute("UPDATE chat_jobs SET created_at=datetime('now','-11 minutes')")
        with patch.dict('os.environ', {'AI_PROVIDER': 'ollama'}):
            self.app.run()
        self.assert_clean()
        text = '\n'.join(m.value for m in self.app.markdown)
        self.assertIn('Saved interrupted question', text)
        self.assertIn('Хүсэлт тасалдсан', text)
        self.assertFalse(self.app.chat_input[0].disabled)

    def test_completed_request_visible_on_same_chat_refresh(self):
        import chat_requests
        self.enter()
        self.app.button(key='nav_chat').click().run()
        session_id = self.app.session_state.active_chat_session_id
        path = self.root / 'dms_system.db'
        self.assertTrue(chat_requests.begin(path, 'completed', session_id, 1, 'Persisted question'))
        self.assertTrue(chat_requests.finish(path, 'completed', 1, 'Persisted answer'))
        self.app.run()
        self.assert_clean()
        text = '\n'.join(m.value for m in self.app.markdown)
        self.assertIn('Persisted question', text)
        self.assertIn('Persisted answer', text)

    def test_cached_startup_repairs_missing_selection_column(self):
        # setUp has already run cached initialization. Simulate an older DB
        # reappearing at the same path without clearing Streamlit's cache.
        with closing(database.open_database()) as conn, conn:
            conn.execute("INSERT INTO chat_sessions(id,user_id,title) VALUES (90,1,'Keep chat')")
            conn.execute("INSERT INTO chat_messages(session_id,role,content) VALUES (90,'user','Keep question')")
            conn.execute('ALTER TABLE chat_sessions DROP COLUMN selection_initialized')
        self.app.run()
        self.assert_clean()
        self.assertFalse(any('Шинэчлэлт дутуу' in e.value for e in self.app.error))
        with closing(database.open_database()) as conn:
            self.assertIn('selection_initialized', {r[1] for r in conn.execute('PRAGMA table_info(chat_sessions)')})
            self.assertEqual(conn.execute('SELECT content FROM chat_messages WHERE session_id=90').fetchone()[0], 'Keep question')

    def test_large_library_paginates_and_search_resets_page(self):
        with closing(database.open_database()) as conn, conn:
            conn.executemany("INSERT INTO documents(title,file_path,status) VALUES (?,'uploaded_files/policy.txt','active')",
                             [(f'Extra {n}',) for n in range(25)])
        self.enter()
        self.assertEqual(len([b for b in self.app.button if str(b.key).startswith('view_')]),12)
        self.app.selectbox(key='library_page').select(3).run()
        self.assertEqual(len([b for b in self.app.button if str(b.key).startswith('view_')]),3)
        self.app.text_input(key='library_search').set_value('Safety').run()
        self.assertTrue(any(b.key == 'view_1' for b in self.app.button))
        self.assert_clean()

    def test_informal_english_document_count(self):
        self.enter()
        with patch.dict('os.environ', {'AI_PROVIDER':'ollama'}):
            self.app.button(key='nav_chat').click().run()
            self.app.chat_input[0].set_value('how many file in there?').run()
        self.assert_clean()
        self.assertTrue(any('There are 2 documents selected' in m.value for m in self.app.markdown))
    ...
    def test_old_provider_shows_update_message_instead_of_type_error(self):
        class OldAIConfig:
            def __init__(self, provider='gemini'):
                self.provider = provider
        with patch('ai_provider.AIConfig', OldAIConfig):
            self.app.run()
            self.assert_clean()
            self.assertTrue(any('Шинэчлэлт дутуу' in item.value for item in self.app.error))
            self.assertTrue(any('ai_provider.py' in item.value for item in self.app.markdown))
    ...
    def enter(self, role="Admin"):
        ...
        if role == "User":
            ...
            with closing(database.open_database()) as conn, conn:
                conn.execute("UPDATE users SET status='active' WHERE username='reader'")
            username = "reader"
    ...
    def test_new_registration_requires_approval(self):
        success, _ = auth.register_user('new-reader','test-password')
        self.assertTrue(success)
        self.assertFalse(auth.login_user('new-reader','test-password')[0])

    def test_empty_chat_selection_survives_navigation(self):
        self.enter()
        self.app.button(key='nav_chat').click().run()
        self.app.button(key='clear_chat_docs').click().run()
        self.assertEqual(self.app.multiselect[0].value, [])
        self.app.button(key='nav_library').click().run()
        self.app.button(key='nav_chat').click().run()
        self.assertEqual(self.app.multiselect[0].value, [])
        self.app.button(key='select_all_chat_docs').click().run()
        self.assertEqual(len(self.app.multiselect[0].value), 2)
        self.assert_clean()
    ...
    def test_admin_pages_and_upload_validation(self):
        self.enter()
        for page in ("upload", "metadata", "users", "recycle", "ai_settings", "chat", "library"):
            self.app.button(key=f"nav_{page}").click().run()
            self.assert_clean()
        self.app.button(key="nav_upload").click().run()
        self.assertTrue(next(b for b in self.app.button if b.label == "Баримтыг байршуулах").disabled)
        self.assert_clean()
    ...
    def test_processing_page_and_content_preview(self):
        self.enter()
        self.assertNotIn('nav_processing', [b.key for b in self.app.button])
        self.app.button(key='processing_1').click().run()
        self.assertTrue(any('Company policy test document.' in t.value for t in self.app.text))
        self.assert_clean()

    def test_empty_drafts_reused_and_title_updates_immediately(self):
        self.enter()
        with patch.dict('os.environ', {'AI_PROVIDER':'ollama'}):
            self.app.button(key='nav_chat').click().run()
            for _ in range(3):
                self.app.button(key='sidebar_new_chat').click().run()
            with closing(database.open_database()) as conn:
                self.assertEqual(conn.execute('SELECT COUNT(*) FROM chat_sessions').fetchone()[0], 1)
            self.app.chat_input[0].set_value('how many file in there?').run()
        self.assert_clean()
        self.assertTrue(any(b.label == 'how many file in there?' for b in self.app.button))

    def test_fresh_answer_source_button_opens_original(self):
        from rag_service import RetrievedSource
        source = RetrievedSource(number=1,document_id=1,title='Safety policy',file_name='policy.txt',
            page_label='Мөр 1',score=1,context_text='Company policy test document.',
            excerpt='Company policy test document.',file_path='uploaded_files/policy.txt')
        self.enter()
        with patch.dict('os.environ', {'AI_PROVIDER':'ollama'}), patch('rag_service.DocumentRAG.answer',return_value=('Answer [1]',[source])):
            self.app.button(key='nav_chat').click().run()
            self.app.chat_input[0].set_value('Explain the safety policy').run()
            next(b for b in self.app.button if b.label == 'Эх хуудас нээх').click().run()
        self.assert_clean()
        self.assertTrue(any(b.label == 'Эх файл татах' for b in self.app.get('download_button')))

    def test_roles_share_library_and_selected_scope_is_visible(self):
        self.enter()
        admin_ids = {b.key for b in self.app.button if str(b.key).startswith('view_')}
        self.app.button(key='nav_chat').click().run()
        self.app.button(key='clear_chat_docs').click().run()
        self.assertTrue(any('сонгогдоогүй' in i.value for i in self.app.info))
        self.app.button(key='use_all_chat_docs').click().run()
        self.assertEqual(len(self.app.multiselect[0].value),2)
        self.enter('User')
        user_ids = {b.key for b in self.app.button if str(b.key).startswith('view_')}
        self.assertEqual(admin_ids,user_ids)
