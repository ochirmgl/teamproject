...
def upload_page():
    ...
    with form_col, st.container(border=True):
        ...
        upload_valid = bool(title.strip() and uploaded is not None and 0 < uploaded.size <= management.MAX_UPLOAD_BYTES)
        if uploaded is not None and Path(uploaded.name).suffix.lower() in {'.doc','.png','.jpg','.jpeg','.webp'}:
            st.warning('Эх файл хадгалагдана. Зурагт OCR, хуучин DOC файлд DOCX хөрвүүлэлт шаардлагатай; одоогоор AI уншихгүй.')
        if st.button("Баримтыг байршуулах", type="primary", use_container_width=True, disabled=not upload_valid):
    ...
    with help_col, st.container(border=True, key="upload_help"):
        ...
        st.caption(f"Дээд хэмжээ: {management.MAX_UPLOAD_MB} MB. Ижил нэртэй файлууд тусдаа хадгалагдана. Текст: UTF-8.")
...
def processing_dialog(document_id):
    ...
    with closing(open_database()) as quality_conn:
        quality_row = quality_conn.execute('SELECT file_path FROM documents WHERE id=?',(document_id,)).fetchone()
    if quality_row:
        try:
            report = inspect_coverage(local_file_path(quality_row[0]), sections)
            if report['source_pages'] is not None:
                st.caption(f"Эх файл: {report['source_pages']} хуудас · Текст задарсан: {report['text_pages']} хуудас")
            for warning in report['warnings']:
                st.warning(warning)
        except Exception:
            st.warning('Эх файлын бүрэн байдлыг шалгаж чадсангүй. Эх файлаа нээж шалгана уу.')
...
def versions_dialog(document_id):
    ...
    for version_id, stored, name, created, current in rows:
        with st.container(border=True):
            display_name = str(name or stored).replace('\\', '/').split('/')[-1]
            st.write(f"{display_name} · {created}" + (" · Одоогийн" if current else ""))
...
def ai_settings_page():
    ...
    user_used = conn.execute("SELECT COUNT(*) FROM ai_requests WHERE user_id=? AND created_at>=date('now')", (current_user_id(),)).fetchone()[0]
    conn.close()
    system_used = sum(row[1] for row in rows)
    st.write(f'Систем: {system_used}/{config.daily_limit} · Үлдсэн: {max(0,config.daily_limit-system_used)}')
    st.write(f'Таны хэрэглээ: {user_used}/{config.user_daily_limit} · Үлдсэн: {max(0,config.user_daily_limit-user_used)}')
    st.caption('Эдгээр хязгаар UTC 00:00 (Улаанбаатар 08:00) цагт шинэчлэгдэнэ. Алдаатай болон хүлээгдэж буй шинэ хүсэлт мөн тоологдоно.')
    st.info('Provider-ийн үлдсэн quota болон шинэчлэгдэх цаг энд тодорхойгүй. Provider-ийн dashboard-аас шалгана уу; системийн үлдэгдэл нь provider-ийн боломжийг батлахгүй.')
...
if st.session_state.logged_in:
    ...
    if page == "library":
        ...
        # Durable history is authoritative after reconnect, recovery or another tab.
        ...
        if notice := st.session_state.pop('chat_submission_notice', None):
            st.warning(notice)
        ...
        if prompt:
            if not prompt.strip() or len(prompt) > 2000:
                st.warning('Асуултаа 1–2000 тэмдэгтэд багтаана уу.')
                st.stop()
            ...
            if not chat_requests.begin(DB_PATH, request_id, active_session_id, user_id, prompt):
                ...
                st.session_state.chat_submission_notice = 'Давхардсан эсвэл өөр хүсэлт боловсруулж байна. Шинэ AI дуудлага илгээгээгүй. Өмнөх хүсэлт дууссаны дараа асуултаа шалгаж дахин илгээнэ үү.'
                st.rerun()
