...
class DatabaseTests(unittest.TestCase):
    def test_selection_migration_preserves_existing_chat(self):
        for version in (5, 6):
            with self.subTest(version=version), tempfile.TemporaryDirectory() as folder:
                path = Path(folder) / 'test.db'
                init_db(path)
                with closing(open_database(path)) as conn, conn:
                    conn.execute("INSERT INTO chat_sessions(id,user_id,title) VALUES (1,1,'Keep this chat')")
                    conn.execute("INSERT INTO chat_messages(session_id,role,content) VALUES (1,'user','Keep this message')")
                    conn.execute('ALTER TABLE chat_sessions DROP COLUMN selection_initialized')
                    conn.execute(f'PRAGMA user_version = {version}')
                init_db(path)
                with closing(open_database(path)) as conn:
                    self.assertEqual(conn.execute('SELECT selection_initialized FROM chat_sessions').fetchone(), (0,))
                    self.assertEqual(conn.execute('SELECT content FROM chat_messages').fetchone(), ('Keep this message',))
                    self.assertEqual(conn.execute('PRAGMA user_version').fetchone(), (6,))
